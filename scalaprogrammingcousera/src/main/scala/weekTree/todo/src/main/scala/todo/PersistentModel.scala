...
object PersistentModel extends Model:
  ...
  val idGenerator = IdGenerator(loadId())
  /**
  ...
  def create(task: Task): Id =
    ...
    val store = loadTasks().toMap + (id -> task)
    saveId(id)
    saveTasks(Tasks(store))
    id
  ...
  def update(id: Id)(f: Task => Task): Option[Task] =
    val store = mutable.LinkedHashMap.from(loadTasks().toList)
    val result = store.updateWith(id)(opt => opt.map(f))
    saveTasks(Tasks(store))
    result
  ...
  def complete(id: Id): Option[Task] =
    val store = mutable.LinkedHashMap.from(loadTasks().toList)
    store(id) = store(id).complete
    saveTasks(Tasks(store))
    store.get(id)
    

  def tags: Tags =
    Tags(loadTasks().toMap.flatMap((id, task)=> task.tags).toSet.toList)
