...
object InMemoryModel extends Model:
  ...
  def complete(id: Id): Option[Task] =
    idStore(id) = idStore(id).complete
    idStore.get(id)
  ...
  def delete(id: Id): Boolean =
    var found = idStore.find((id_task, task) => id_task==id)
    found match
      case Some(id, task) => {
        idStore.remove(id)
        true
  ...
  def tags: Tags =
    Tags(idStore.flatMap(x => x._2.tags.toList).toSet.toList)

  def tasks(tag: Tag): Tasks =
    val tmpIdStore  = idStore.filter( (id_task, task) => task.tags.contains(tag))
    Tasks(tmpIdStore)
